...
// We need to import the actual functions from middleware.ts for testing
// Since the middleware.ts file doesn't export individual functions, we'll test them through the main middleware function
// But first, let's create unit tests for the logic that should be testable
...
describe('Authentication Middleware', () => {
  ...
  beforeEach(() => {
    ...
    // Create mock request
    mockRequest = {
      nextUrl: { pathname: '/test', search: '', searchParams: new URLSearchParams() },
      method: 'GET',
      ip: '127.0.0.1',
      headers: new Map(),
      cookies: new Map(),
  ...
  describe('Rate Limiting Logic', () => {
    // Since we can't directly test internal functions, we'll test the behavior through integration

    it('should create proper rate limiting store structure', () => {
      // Test that rate limiting logic would work with proper data structure
  ...
  describe('Security Headers Logic', () => {
    ...
    it('should apply security headers to response', () => {
      const applySecurityHeaders = (response: NextResponse, headers: Record<string, string>) => {
      ...
      const updatedResponse = applySecurityHeaders(response, securityHeaders);
